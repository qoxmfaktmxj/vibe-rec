...
HireFlow의 시각 세계는 지원의 전 과정을 하나의 커리어 시그널로 읽게 한다. 장식적인 SaaS 카드 모음 대신 강한 색면, 실제 공고 데이터, 단계와 경로, 정렬된 인덱스를 사용해 발견부터 지원과 상태 확인까지 같은 방향 감각을 유지한다.
...
- **Signal White:** 블루 위 제목, 단계 경로와 라벨에 사용해 정보가 색면에서 또렷하게 읽히게 한다.
...
HireFlow는 색면, 표면 톤, 1px 경계를 먼저 사용하고 그림자를 상태나 중요한 부유 요소에만 더한다. 공개 히어로와 공고 인덱스는 기본적으로 평평하며, 현재 단계 노드와 핵심 행동만 주변 잉크를 머금은 짧은 그림자를 사용한다. 관리자 카드와 팝오버는 기존 3단계 잉크 틴트 그림자를 유지한다.
...
- **Stage Node** (`0 10px 24px -14px var(--signal-ink)`): 전형 단계 노드를 시그널 색면에서 가볍게 분리한다.
...
운영 컨트롤과 카드에는 절제된 곡률을 사용하고, 공개 신호 세계에는 원과 직선형 경로를 명확한 문법으로 사용한다. 기본 버튼과 입력은 8px, 큰 카드와 패널은 약 11px이다. `rounded-2xl`과 `rounded-3xl` 같은 부풀린 컨테이너는 사용하지 않는다.
...
**The Stage and Index Rule.** 직선형 단계 경로는 진행과 연결을, 직선형 인덱스는 비교를 표현한다. 두 형태의 역할을 바꾸거나 모든 컨테이너를 pill로 만들지 않는다.
...
**The Motion Contract Rule.** 모션은 경로와 상태 변화를 설명하는 데만 사용한다. `LazyMotion`과 `MotionConfig`의 사용자 모션 감소 설정을 유지하고, `prefers-reduced-motion`에서는 단계 경로, entrance, layout, hover transform, 자식 화살표 이동을 모두 정지한다.
